...
dict_len = {key: len(value) for key, value in dico.items()}
sorted_key_list = sorted(dict_len.items(), key=operator.itemgetter(1), reverse=True)
sorted_dict = [{item[0]: dico[item [0]]} for item in sorted_key_list]
d = sorted_dict
...
for i in range(len(sorted_dict)):
    ...
    elem = x.get(key)
    nodes.append(key)
    neighboors.append(elem)

for i in range(len(nodes)):
    if (len(nodes) > 0):
        x = nodes[0]
        tempColorizedNodes.append(x)
        coloredNodes.append(x)
        nodes.pop(0)
        neighboors.pop(0)
    for j in neighboors:
        if x not in j:
            coloredNodes.append(j)
            tempColorizedNodes.append(nodes[neighboors.index(j)])
            nodes.pop(neighboors.index(j))
            neighboors.pop(neighboors.index(j))
    if tempColorizedNodes == []:
        break
    coloredDico[cnt] = tempColorizedNodes
    cnt += 1
    tempColorizedNodes = []
...
with open("C:\\Users\\PC\\Desktop\\list2dict.txt") as myfile:
    fstr = myfile.readlines()
with open("C:\\Users\\PC\\Desktop\\bool2table.txt") as myfile:
    fstr2 = myfile.readlines()
#print(fstr)

def graphematrix(fstr2):
    with open("C:\\Users\\PC\\Desktop\\bool2table.txt") as myfile:
        fstr = myfile.readlines()
    readArray = fstr
    matrix = []
...
file_dir = os.path.dirname("C:\\Users\\PC\\Desktop\\123\\modulePlot.py")
